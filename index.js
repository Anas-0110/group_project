...
const uri = "mongodb+srv://Anas_Ikhwan:Iapqe2EjMFsOmVCL@cluster0.jgb94.mongodb.net/";
...
app.get('/', async (req, res) => {
  res.send('Hello Welcome To MyTaxi!');
});

// Registration route
app.post('/register', async (req, res) => {
  const user = await client.db("anas").collection("project").findOne(
  ...
  await client.db("anas").collection("project").insertOne({
    "name": req.body.name,
...
app.get('/admin/login', async (req, res) => {     //login for admin
  try {
    const user = await client.db("anas").collection("project").findOne(
...
app.get('/admin/users', verifyToken, checkRole('admin'), async (req, res) => {    //view all users
  try {
    const users = await client.db("anas").collection("project").find({ role: { $in: ['driver', 'passenger'] } }).toArray();
    res.json(users);
  } catch (error) {
...
app.get('/admin/rides', async (req, res) => {
  try {
    const rides = await client.db("anas").collection("project2").find().toArray();
    res.json(rides);
  } catch (error) {
...
app.put('/admin/users/:id', verifyToken , async (req, res) => {     //update admin details
   ...
   await client.db("anas").collection("project").updateOne({ _id: ObjectId} , {$set: updatedUser });
   res.send('User updated'); 
});

app.delete('/admin/users/:id', verifyToken, checkRole('admin'), async (req, res) => { //delete admin id
  const userId = req.params.id;

  try {
    const result = await client.db("anas").collection("project").deleteOne({ _id: new ObjectId(userId) });
...
app.post('/driver/register/car' , async (req, res) => {
  try {
    const existingUser = await client.db("anas").collection("project").findOne(
      { username: req.body.username }
    );
    ...
    const result = await client.db("anas").collection("project").insertOne(newDriver);
    ...
    await client.db("anas").collection("project3").insertOne(newCar);
...
app.get('/driver/passengers', verifyToken, checkRole('driver'), async (req, res) => {
  try {
    const passengers = await client.db("anas").collection("project").find({ role: 'passenger' },{ projection: { name: 1 } }).toArray();
    res.json(passengers);
  } catch (error) {
...
app.put('/driver/profile', verifyToken, checkRole('driver'), async (req, res) => {
  ...
  const user = await client.db("anas").collection("project").findOne({ _id: new ObjectId(driverId) });

  if (!user) {
    return res.status(404).send('User not found');
  }

  await client.db("anas").collection("project").updateOne({ _id: new ObjectId(driverId) }, { $set: updatedUser });
...
app.delete('/driver/account', verifyToken, checkRole('driver'), async (req, res) => {
  try {
    ...
    const user = await client.db("anas").collection("project").findOne({ username });
    ...
    const result = await client.db("anas").collection("project").deleteOne({ username });
...
app.post('/driver/accept-ride', verifyToken, checkRole('driver'), async (req, res) => {
  try {
    ...
    const ride = await client.db("anas").collection("project2").findOne({ _id: new ObjectId(rideId), status: 'requested', driverId: null });

    if (ride) {
      // Update the ride to "accepted" and assign the driver's ID
      await client.db("anas").collection("project2").updateOne({ _id: new ObjectId(rideId) }, {$set: { status: 'accepted',driverId: new ObjectId(driverId) }});
...
app.post('/passenger/request-ride', verifyToken, checkRole('passenger'), async (req, res) => {
  ...
  try {
    const result = await client.db("anas").collection("project2").insertOne(ride);
...
app.post('/passenger/ride-details', verifyToken, checkRole('passenger'), async (req, res) => {
  try {
    ...
    const ride = await client.db("anas").collection("project2").findOne({ _id: new ObjectId(rideId) });
    ...
    if (ride.driverId) {
      const car = await client.db("anas").collection("project3").findOne({ driverId: new ObjectId(ride.driverId) });
...
app.delete('/passenger/account', verifyToken, checkRole('passenger'), async (req, res) => {
  try {
    ...
    const user = await client
      .db("anas").collection("project").findOne({ username });
